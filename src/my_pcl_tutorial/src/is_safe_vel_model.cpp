//Once there is an unsafe configuration, pub_nav_odom will get stuck at the last safe configuration
//pub_bool publishes whether a given configuration is safe or not
#if 0
#include <ros/ros.h>
...
bool is_fixed_global = 0;//have we already fixed nav_odom_msg's velocities(to be publlished) to be zero once it is known that it is now unsafe
...
void findCarTraj()
{
  float r_circle = v_tangential/v_angular;
  float x_coord=0;
  float y_coord=0;
  ...
  if(v_angular > 0)
  {
    for(int i=0;i<num_time_divisions;i++){
      temp_triplet.x = x_coord; temp_triplet.y = y_coord; temp_triplet.z = 0.0;
      car_trajectory.push_back(temp_triplet);
      y_coord =  -r_circle + r_circle*cos(i*v_angular*time_inderval);
      x_coord =  r_circle*sin(i*v_angular*time_inderval);    
    }
  }
